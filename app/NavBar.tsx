...
export default function Navbar() {
  ...
  return (
    <Flex justifyContent={"space-between"}>
      ...
      <Flex gap={2}>
        <IconButton icon={<RepeatIcon />} onClick={() => router.refresh()} aria-label={""} />
        <IconButton icon={<ArrowBackIcon />} onClick={() => router.back()} aria-label={""} />
        <IconButton
                  icon={<ArrowForwardIcon />}
                  onClick={() => router.forward()} aria-label={""}/>
      </Flex>
    </Flex>
  );
}